...
## Browser demo
- `demo/` is served at https://receipt-demo.stoatworks-labs.com/ by this repo's Worker,
  through a proxied AAAA `100::` DNS record + a route (the zone is out of custom domains;
  deleting the record takes the page dark with green deploys). A push to main deploys it
  (`deploy.yml`); by hand: `cf-run npx wrangler deploy`. Verify by content:
  `curl -s 'https://receipt-demo.stoatworks-labs.com/?cb=1' | grep -o '<title>[^<]*'`
...
- `demo/printer.js` is a hand PORT of `Model.h`, `Controls.cpp`, `Printer.cpp` and the CPU
  half of `Receipt.cpp`. `demo/tools/check_port.sh` compares it with the plugin's own C++
  exactly (it cuts `Receipt.h`'s members and `Receipt.cpp`'s feed and `ProcessOpenGL` out
  of the source at run time, so renaming or moving those breaks the check loudly).
  Change the engine, the feed or a control law: change printer.js too. The GL calls in
  `plugin.js` only a reader checks.
- `demo/vendor/` is the shared kit: never edit it; re-run
  `stoatworks-backend/resolume-demo/sync.sh receipt`.
...
- Windows: CI builds it; the release DLL passed the Arena gate 9/9 on win-lab (software
  rendering). No OpenFX port, no presets.
...
- The user guide is `docs/USER-GUIDE.md`; `docs/USER-GUIDE.pdf` is generated by the
  website's `build_guides.py receipt`.
