...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    host's picture changes size, the head's width or the Fit. A reallocation
    clears, which is safe because the only PassBuffer here is the head's
    image, which is sampled afresh every frame. The paper that must survive
    a resize is NOT a PassBuffer: it is held at head resolution on the CPU
    and in a texture whose size depends only on the head (see `Receipt.h`).
    ...
    **It owns its filtering.** Every buffer in this plugin is read texel for
    texel with `texelFetch` or read back whole, so every buffer here is
    Nearest. Linear and Mipmapped are tinsel's and are unused.

    Copied from tinsel (by way of toner), where it was written for a glow
