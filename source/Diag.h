...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; this records *which* of the two passes it was. The GL
    ...
    **A buffer that could not be allocated.** The head's image (N dots by up
    to 8192 rows) and the paper ring (N by 4096 rows, 9.4 MB at 80 mm) are
    R32F, and the failure mode when the driver says no is a black frame with
    nothing to explain it. What goes in the log is the size asked for.
*/
namespace receiptfx::diag
{
